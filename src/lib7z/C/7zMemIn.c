...
static SRes MemInStream_Skip(ILookInStreamPtr pp, size_t offset)
{
  CMemInStream *p = (CMemInStream *)pp;
  p->pos += offset;
  return SZ_OK;
...
static SRes MemInStream_Read(ILookInStreamPtr pp, void *buf, size_t *size)
{
  CMemInStream *p = (CMemInStream *)pp;
  size_t remaining = p->end - p->pos;
  if (remaining == 0)
...
static SRes MemInStream_Seek(ILookInStreamPtr pp, Int64 *pos, ESzSeek origin)
{
  CMemInStream *p = (CMemInStream *)pp;
  switch (origin)
  {
